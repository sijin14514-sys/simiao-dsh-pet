...
## 六、DSH 联动的"工作状态"动画（v1.3.2 起）

`dsh-pet-indesktop` 支持与 AI Agent 联动：agent 干活时桌宠会切换动作。它的识别规则是**看动画文件名**：

| 池 | 文件名关键词 | 行为 |
|---|---|---|
| 忙碌池 | `代码` / `工作` / `写` / `打字` / `敲` | agent 处于 working / thinking 时轮换播放 |
| 摸鱼池 | `记录` / `踏步` / `伸懒腰` | 每第 3 次插播一次"摸鱼" |

如果角色一条都不匹配，它会**退回用整个 random 池轮换**——看起来和平时的小动作没区别，等于看不出联动。

所以本包在 `videos/random/` 里带了一套按该规则命名的动画（直接复用现有素材，非新素材）：

| 文件名 | 复用自 | 语义 |
|---|---|---|
| `工作状态-看代码.webm` | 看书 | 低头阅读 / 审阅 |
| `工作状态-忙碌擦汗.webm` | 擦汗 | 忙到出汗 |
| `工作状态-戴耳机专注.webm` | 戴耳机听歌 | 戴耳机专注干活 |
| `伸懒腰-摸鱼吃零食.webm` | 吃零食 | 摸鱼 |
| `伸懒腰-打盹.webm` | 打瞌睡 | 摸鱼 |

这样接入 DSH（或其它 Agent）联动后，**忙碌期间她播的就是"工作状态"那套**，一眼能看出联动生效。

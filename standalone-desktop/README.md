...
安装脚本会自动开启三件事，让她在工作时开口：

| 能力 | 说明 |
|---|---|
| **自言自语** | 空闲时每 8~18 秒说一句（8 条肆喵口吻台词），气泡配图用她自己的立绘 |
| **干活汇报** | Agent 每做一步（读文件 / 跑命令 / 改代码…）冒一句，例如「{name}正在跑{label}，结果很快就出来咯～」 |
| **自定义台词** | `dialogue_mode = custom`，30 个事件 / 88 条文案，按事件分类（开始 / 思考 / 读 / 搜 / 改 / 跑 / 完成 / 报错 / 审批 / 提问…） |

三处相关配置（都写进应用用户目录的 `config.json`，**UTF-8 无 BOM**）：

- `dialogue_mode: "custom"` 与 `dialogue_phrases.global`（台词本体，源文件见 `talk/simiao-phrases.json`）
- `agent_link.report_gates.activity: 1.0`：过程汇报不再按 60% 抽稀
- `self_talk_enabled: true` + `self_talk_texts` + `self_talk_image_dir`（气泡配图目录，装到 `<用户目录>/simiao-faces/`）

> ⚠️ 过程汇报有**三重节流**（上游设计）：同一 Agent 10 秒 / 全局 8 秒 / **同一个工具文案 60 秒内不重复**——
> 所以连着用同一个工具只会说一次。
>
> 不想让她说话：把 `self_talk_enabled` 改成 false（或在桌宠设置里关掉「自言自语」），
> 并把 `report_gates.activity` 调低即可。
